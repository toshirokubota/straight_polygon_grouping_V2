...
public:
   ...
   matrix (size_t row, size_t col, const T& e);
   ...
   size_t Row() const { return _m->Row;}
   size_t Col() const { return _m->Col;}
   size_t RowSiz() const {return _m->RowSiz;}
   size_t ColSiz() const {return _m->ColSiz;}
   T* operator[] (int i) {
	   if(i>=0 && i<Row())
		   return _m->Val[i];
	   else
...
private:
	struct base_mat
	{
		...
		base_mat& operator = (const base_mat& m) _NO_THROW
		{
			~base_mat();
			Row = m.Row; RowSiz = m.RowSiz;
			Col = m.Col; ColSiz = m.ColSiz;
			Refcnt = 1;
...
MAT_TEMPLATE inline
matrixT::matrix (size_t row, size_t col, const T& e)
{
  _m = new base_mat( row, col, 0);
  for(size_t i=0; i<Row(); ++i)
  {
...
matrixT::operator = (const matrixT& m) _NO_THROW
{
    ...
    if (--_m->Refcnt == 0) 
		delete _m;
	//_m = m._m;
	_m = new base_mat(m.Row(), m.Col(),0);
	size_t rowlen = _m->Col * sizeof(T);
	for (size_t i=0; i < _m->Row; i++)
	{
		memcpy( _m->Val[i], m[i], rowlen);
	}
    return *this;
}
...
matrixT::operator += (const matrixT& m) _THROW_MATRIX_ERROR
{
   ...
   for (size_t i=0; i < m._m->Row; i++)
	  ...
	  {
		  //cout << "+=:: " << _m->Val[i][j] << " + " << m._m->Val[i][j];
		  _m->Val[i][j] += m._m->Val[i][j];
		  //cout << " = " << _m->Val[i][j] << endl;
	  }
   return *this;
}
