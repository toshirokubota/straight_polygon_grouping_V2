...
StronglyConnectedComponents(vector<Vertex<T>*>& vnodes)
{
	...
	//Step 2: compute G^T - reversal of edges
	reverseEdges(vnodes);
	...
	for(int i=0; i<vnodes.size(); ++i)
	{
		...
		{
			vector<Vertex<T>*> trace = DFS(vnodes[index], time);
	...
	//Step 4: transposing it back to restore the original
	reverseEdges(vnodes);
	return components;
}
...
template <class T>
vector<int>
StronglyConnectedComponentsAdjacency(vector<Vertex<T>*>& vnodes, const AdjacencyMatrix& adj)
{
	//Step 1: call DFS to compute finishing times for each vertex
	for (int i = 0; i<vnodes.size(); ++i)
	{
...
template <class T>
vector<int>
StronglyConnectedComponentsA(vector<Vertex<T>*>& vnodes)
{
	...
	return StronglyConnectedComponentsAdjacency(vnodes, adj);
}
