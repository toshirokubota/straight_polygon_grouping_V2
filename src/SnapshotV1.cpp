...
mxArray*
Snapshot::StoreSnapshotsOrderedByArea(vector<Snapshot>& snapshots)
{
	ParticleFactory* factory = ParticleFactory::getInstance();
	...
	for (int i = 0; i < snapshots.size(); ++i)
	{
		...
		{
			MovingParticle* p = factory->get(snapshots[i].particles[j]);
			CParticleF p0 = p->project(Max(p->getCreatedTime(), snapshots[i].time));
			pnts.push_back(p0);
		}
		pair<float, int> pr(polygonArea(pnts), i);
		pairs.push_back(pr);
	}
	...
	for (int i = 0; i<snapshots.size(); ++i)
	{
		...
		mxArray* a = StoreSnapshot(snapshots[j]);
		mxSetCell(plhs, i, a);
	}
	return plhs;
}
