...
float distanceAsymmetric(MAPoint* p, MAPoint* q)
{
	...
	CParticleF q0(q->x, q->y, q->z); //disregard Z
...
void
buildDag(vector<Edge<MAPoint*>*>& mst, GraphFactory<MAPoint*>& factory)
{
	map<Vertex<MAPoint*>*, int> vmap;
	for (int i = 0; i < factory.vertices.size(); ++i)
	{
		Vertex<MAPoint*>* u = factory.vertices[i]; 
		u->aList.clear();
		vmap[u] = i;
	}
	for (int i = 0; i < mst.size(); ++i)
	{
		...
		{
			mst[i]->v->Add(factory.makeEdge(mst[i]->v, mst[i]->u, mst[i]->w));
		}
	}
}

vector<int>
clusterDag(GraphFactory<MAPoint*>& factory)
{
	...
	for (int i = 0; i < factory.vertices.size(); ++i)
	{
		...
		{
			MAPoint* q = u->aList[j]->v->key;
			int k = pmap[q];
			merge(nodes[i], nodes[k]);
		}
	}
	vector<Node<MAPoint*>*> reps = clusters(nodes);
	map<MAPoint*, int> pmap2;
	for (int i = 0; i < reps.size(); ++i)
	{
		MAPoint* p = reps[i]->key;
		pmap2[p] = i;
	}

	vector<int> labels(factory.vertices.size(), -1);
	for (int i = 0; i < factory.vertices.size(); ++i)
	{
		MAPoint* p = factory.vertices[i]->key;
		labels[i] = pmap2[findset(nodes[i])->key];
	}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	buildDag(mst, factory);
	vector<int> labels = clusterDag(factory);
