...
*/
void
DelineateContour(ByteImage& flag, Cpoint* pnts, int nump, int band, 
...
void
DelineateContour(ByteImage& flag, vector<Cpoint>& pnts, int nump, int band, 
				 real scale, unsigned char val) {
	int i;
	for(i=0; i<nump; ++i) {
		real y=pnts[i].GetY();
...
DelineateApproximateContour(ByteImage& flag, Cpoint* pnts, int nump, 
	...
	for(int i=0; i<nump; ++i) {
		...
		DrawLine(flag,band1,iy1,ix1,iy2,ix2,val);
		//DrawCross(flag,band2,iy1,ix1,5,val);
	}
}
...
OutputVectorCpoints(vector<vPoint>& plink, char* filename) {
	...
	for(i=0; i<plink.size(); ++i) {
		out << plink[i].size() << endl;
		for(j=0; j<plink[i].size(); ++j) {
			out << plink[i][j];
		}
	}
	out.close();
