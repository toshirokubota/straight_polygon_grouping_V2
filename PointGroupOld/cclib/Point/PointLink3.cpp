...
bool
FindNextPointNew(const ByteImage& flag, 
				 int y, int x, int& y2, int& x2, bool first) {
	if(flag.GetPixel(0,y,x) && !first)
		return false;  //reached a junction - break it
	int cnt=0;
	int max=0;
	for(int i=y-1; i<=y+1; ++i) {
		for(int j=x-1; j<=x+1; ++j) {
			if(!(i==y && j==x)) {
				if(flag.GetPixelDefault(0,i,j,0)>max) {
					max=flag.GetPixel(0,i,j);
					y2=i;
					x2=j;
					cnt++;
				}
...
void
Track(vPoint& vp, ByteImage& flag, int y, int x) {
	if(flag.GetPixel(0,y,x)>0)
		flag.AddPixel(0,y,x,-1);
	int y2,x2;
	vp.push_back(Cpoint((real)x,(real)y,0,0,0,0));
	int n=0;
	if(FindNextPointNew(flag,y,x,y2,x2,false)) {
		Track(vp,flag,y2,x2);
	}
}
